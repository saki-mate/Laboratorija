...
<br><br>
<!-- ---------------------------------------------------------------------------------------------- -->
<form action ="/Laboratorija/laborantController/vidiBlizeKlijenta" method = "POST">
	<select name = "cbKlijent">
		<c:forEach var="k" items = "${klijenti}">
			<option value = "${k.idKorisnik}">${k.ime} ${k.prezime}</option>
		</c:forEach>
	
	</select><br><br>
	<input type = "submit" value = "odaberi"><br>
</form>
<!-- 
<table class="centar">
    <tr>
    ...
    <c:forEach items="${klijenti}" var="k">
        <tr>
            <td>${k.ime}</td>
            <td>${k.prezime}</td>
            <td>${k.kontakt}</td>
